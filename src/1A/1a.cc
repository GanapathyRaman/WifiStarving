...
main (int argc, char *argv[]){
  ...
  /* Setting maximum transmission range to 10m (Generall Wifi distance) 
 ...
 /* Creating NetDevice Containers */
  ...
  MAC.SetType ("ns3::StaWifiMac","Ssid", SsidValue (ssidAp_A),
               "ActiveProbing", BooleanValue (false));         /* Assigning an SSID to AP_A */
  ...
  MAC.SetType ("ns3::StaWifiMac","Ssid", SsidValue (ssidAp_B),
               "ActiveProbing", BooleanValue (false));         /* Assigning an SSID to AP_B */
  ...
  ApplicationContainer ServerApp = Server.Install (StNodes.Get (1)); /* ST_B as Server */
  ServerApp.Start (Seconds (1.0)); /* Starting time of Server */
  ServerApp.Stop (Seconds (100.0)); /* Stopping time of Server */

  UdpClientHelper Client (StInterfaces.GetAddress (1), 10); /* Binding the Client to Sever and opening Port 10 for communication with server */
  Client.SetAttribute ("MaxPackets", UintegerValue (100000000));
  Client.SetAttribute ("Interval", TimeValue (Seconds (0.0001)));
  Client.SetAttribute ("PacketSize", UintegerValue (1024));

  ApplicationContainer ClientApp = Client.Install (ApNodes.Get (1)); /* AP_B as Client */
