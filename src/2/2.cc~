/* Topology
 ...
 --- 0  |ST-A|      |ST-B|      |ST-C|
	...
	| Access Point  |    AP_A    | 00:00:00:00:00:01 | 192.168.1.1 |
        ...
        +---------------+------------+-------------------+-------------+
        | Access Point  |    AP_C    | 00:00:00:00:00:05 | 192.168.1.3 |
        +---------------+------------+-------------------+-------------+
        | Station Point |    ST_A    | 00:00:00:00:00:02 | 192.168.1.4 |
        +---------------+------------+-------------------+-------------+
        | Station Point |    ST_B    | 00:00:00:00:00:04 | 192.168.1.5 |
        +---------------+------------+-------------------+-------------+
        | Station Point |    ST_C    | 00:00:00:00:00:06 | 192.168.1.6 |
        +---------------+------------+-------------------+-------------+
...
main (int argc, char *argv[]){
 ...
 /* Creating NetDevice Containers */
  ...
  /* Setting up the Network between AP_B and ST_B.
     Assigning the SSID to AP_B and associating ST_B with AP_B.
     Assigning MAC Address to the devices in order */
  Ssid ssidAP_C = Ssid ("ssidAP_C");
  MAC.SetType ("ns3::ApWifiMac","Ssid", SsidValue (ssidAP_C)); /* Assigning an SSID to AP_C */
  ApDevices.Add (wifi.Install (Phy, MAC, ApNodes.Get(2)));     /* Set the MAC address of AP_C to 00:00:00:00:00:05 */
  MAC.SetType ("ns3::StaWifiMac","Ssid", SsidValue (ssidAP_C),
               "ActiveProbing", BooleanValue (false));         /* Assigning an SSID to AP_C */
  StDevices.Add (wifi.Install (Phy, MAC, StNodes.Get(2)));     /* Set the MAC address of AP_C to 00:00:00:00:00:06 */
  ...
  StAlloc->Add(Vector(0.0, 0.0, 0.0));  /* Placing ST_A */
  StAlloc->Add(Vector(7.0, 0.0, 0.0));  /* Placing ST_B */
  StAlloc->Add(Vector(14.0, 0.0, 0.0)); /* Placing ST_C */
  ...
  ApAlloc->Add(Vector(0.0, 7.0, 0.0));  /* Placing ST_A */
  ApAlloc->Add(Vector(7.0, 7.0, 0.0));  /* Placing ST_B */
  ApAlloc->Add(Vector(14.0, 7.0, 0.0)); /* Placing ST_C */
  ...
  /* Server and Client part. 
     AP_C sends data packets to ST_C */ 
  UdpServerHelper Server3 (10);  /* Starting Server at Port 10 */

  ApplicationContainer ServerApp3 = Server3.Install (StNodes.Get (2)); /* ST_C as Server */
  ServerApp3.Start (Seconds (1.0));  /* Starting time of Server */
  ServerApp3.Stop (Seconds (100.0)); /* Stopping time of Server */

  UdpClientHelper Client3 (StInterfaces.GetAddress (2), 10); /* Binding the Client to Sever and opening Port 10 for communication with server */
  ...
  ApplicationContainer ClientApp3 = Client3.Install (ApNodes.Get (2)); /* AP_C as Client */
  ClientApp3.Start (Seconds (2.0));  /* Starting time of Server */
  ClientApp3.Stop (Seconds (100.0)); /* Stopping time of Server */
