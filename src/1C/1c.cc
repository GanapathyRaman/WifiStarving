...
main (int argc, char *argv[]){
  ...
  ApplicationContainer ServerApp1 = Server1.Install (ApNodes.Get (0)); /* ST_A as Server */
  ServerApp1.Start (Seconds (1.0));
  ServerApp1.Stop (Seconds (100.0));

  UdpClientHelper Client1 (ApInterfaces.GetAddress (0), 10); /* Binding the Client to Sever and opening Port 10 for communication with server */
  Client1.SetAttribute ("MaxPackets", UintegerValue (100000000));
  Client1.SetAttribute ("Interval", TimeValue (Seconds (0.0001)));
  Client1.SetAttribute ("PacketSize", UintegerValue (1024));

  ApplicationContainer ClientApp1 = Client1.Install (StNodes.Get (0)); /* AP_A as Client */
